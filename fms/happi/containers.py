...
class LCLSItem(OphydItem):
    ...
    location_group = EntryInfo(
        "LUCID grouping parameter for location", optional=False, enforce=str
    )
    functional_group = EntryInfo(
        "LUCID grouping parameter for function", optional=False, enforce=str
    )
    z = EntryInfo("Beamline position of the device", enforce=float, default=-1.0)
    stand = EntryInfo(
    ...
    input_branches = EntryInfo(
        ("List of branches the device can receive " "beam from."),
        optional=True,
        enforce=list,
    )
    output_branches = EntryInfo(
        ("List of branches the device can deliver " "beam to."),
        optional=True,
        enforce=list,
    )
    ioc_engineer = EntryInfo(
        ("Engineer for the IOC. Used to build IOC " "configs."),
        optional=True,
        enforce=str,
    )
    ioc_location = EntryInfo(
        "Location of the IOC. Used to build IOC configs.", optional=True, enforce=str
    )
    ioc_hutch = EntryInfo(
        ("Hutch the IOC will be used in. Used to build IOC " "configs."),
        optional=True,
        enforce=str,
    )
    ioc_release = EntryInfo(
        ("Full path to IOC release directory. Used to " "build IOC configs."),
        optional=True,
        enforce=str,
    )
    ioc_arch = EntryInfo(
        ("The IOC host architecture. Used to build IOC " "configs."),
        optional=True,
        enforce=str,
    )
    ioc_name = EntryInfo(
        "The name of the device IOC. Used to build the IOC.", optional=True, enforce=str
    )
    ioc_type = EntryInfo(
        (
            "The type of device IOC. Useful when multiple "
...
class FMSRaritanItem(FMSItem):
    ...
    root_sensor_port = EntryInfo(
        "switch port root sensor connected to 0-8",
        optional=True,
        default=None,
        enforce=re.compile(r"^[0-8]$"),
    )

    parent_switch = EntryInfo("The raritan switch", optional=False, enforce=str)

    eth_dist_last = EntryInfo(
        "Estimate the ethernet distance from last connection in feet",
        enforce_doc="must be less than 100 feet total from switch to last sensor",
        optional=False,
        enforce=less_than_100,
    )

    last_connection_name = EntryInfo(
        "Name of last connected sensor or leave blank for first", enforce=str
    )

    num_sensors = EntryInfo(
        "DX2-T1H1: 3, DX2-T1: 1, DX2-WSC: 1", optional=False, enforce=int
    )
